...
import { ArrowLeft, Search, Loader2, ChevronLeft, ChevronRight, Eye, EyeOff, PlusCircle, Trash2, RefreshCw, Music, Image, FileText, Upload, Users, Package } from "lucide-react";
import { useAuth } from "@/context/AuthContext";
import { Navbar } from "@/components/Navbar";
import { GiftRequestsManager } from "@/components/GiftRequestsManager";
...
const AdminDashboard = () => {
    const { token } = useAuth();
    const [activeView, setActiveView] = useState<"users" | "requests">("requests"); // Default to requests view
    ...
    return (
        <div className="min-h-screen bg-background flex flex-col items-center p-4 relative overflow-hidden">
            ...
            <div className="w-full max-w-5xl space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pt-28">
                {/* Top bar: title, tabs, and search (sticky below navbar) */}
                <div className="sticky top-16 lg:top-20 z-40 bg-background/80 backdrop-blur-md border-b border-white/20 py-3 space-y-3">
                    {/* Title and Tabs */}
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                        ...
                        <div className="flex gap-2 p-1 bg-muted/30 rounded-xl border border-border/50">
                            <button
                                ...
                                className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all ${activeView === "requests"
                                    ? "bg-gold text-gold-foreground shadow-md"
                                    : "hover:bg-muted/50 text-muted-foreground"
                                    }`}
                            >
                                <Package className="w-4 h-4" />
                                <span className="font-medium">Gift Requests</span>
                            </button>
                            <button
                                onClick={() => setActiveView("users")}
                                className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all ${activeView === "users"
                                    ? "bg-gold text-gold-foreground shadow-md"
                                    : "hover:bg-muted/50 text-muted-foreground"
                                    }`}
                            >
                    ...
                    {/* Search bar - only show for Users view */}
                    {activeView === "users" && (
                ...
                {/* Conditional View: Users or Gift Requests */}
                {activeView === "users" ? (
                    <>
                        {/* Users List */}
                    ...
                    /* Gift Requests View */
                    <div className="bg-white/50 backdrop-blur-xl border border-white/20 rounded-2xl p-6 shadow-soft">
                        <GiftRequestsManager token={token} />
                    </div>
                )}
